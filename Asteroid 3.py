...
class rock(pygame.sprite.Sprite):
    def __init__(self,colour):
        ...
        self._gravity = 5000
        self._colour = colour
        self._size = random.randint(xpixel_internal//100,xpixel_internal//70)
...
class laser(rock):
    def __init__(self,vector,Guy):
        ...
        self.rect.height,self.rect.width = 3,3
    def update(self,Guy):
...
while Guy._health>0:
    ...
    if pressed()[pygame.K_SPACE] == 1:
        print(clock.get_fps())
        if time.time() - Last_laser > 0.01:
            if pressed()[pygame.K_LEFT]:
                TempLaser =laser([-Guy._vector[0]-5,-Guy._vector[1]],Guy)
                Fired = True
                ...
                TempLaser =laser([-Guy._vector[0],-Guy._vector[1]-5],Guy)
                Fired = True
